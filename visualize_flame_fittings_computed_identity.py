...
def visualize_fittings_on_seq(sub_seq_name, postfix=""):
    ...
    acc_file = os.path.join(dataset_path, "subj_seq_fitting_results_w_mean_shape/accumulated_extracted_wmesh_ns473.pkl")
    ...
    for f_id in range(num_verts):
        ...
        # with mean shape
        face_with_exprs_meanshape_pose = get_flame_face_given_expression(flamelayer, current_exprs, jaw_pose, current_mean_shape_params,
                                                                    neck_pose,
                                                                    eye_pose
                                                                    )[0]
        renderd_face_with_exprs_meanshape_Fullpose = render_image(mesh_render, face_with_exprs_meanshape_pose, flames_faces, "face_with_exprs_meanshape_pose")
        ...
        # with mean shape, exprs and jawpose
        face_with_exprs_meanshape_jawpose = get_flame_face_given_expression(flamelayer, current_exprs, jaw_pose, current_mean_shape_params)[0]
        renderd_face_with_exprs_meanshape_jawpose = render_image(mesh_render, face_with_exprs_meanshape_jawpose, flames_faces, "face_with_exprs_meanshape_jawpose")
        ...
        # compose image
        top_row = np.concatenate([renderd_gt_mesh, renderd_optim_mesh, renderd_face_with_exprs_shape_Fullpose, renderd_face_with_exprs_meanshape_Fullpose], axis=1)
        botton_row = np.concatenate([renderd_face_with_exprs_shape_jawpose, renderd_face_with_exprs_meanshape_jawpose, renderd_face_with_exprs_noshape_jawpose, renderd_face_with_exprs_noshape_nopose], axis=1)
...
if __name__ == "__main__":
    # run_on_accumulate_dataset(postfix="_mean_shape_check")
    sub = "FaceTalk_170811_03275_TA_sentence10"
    visualize_fittings_on_seq(sub, postfix="_mean_shape_check")