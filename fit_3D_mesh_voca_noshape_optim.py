...
def fit_3D_mesh_with_init(vertices, mean_tf_shape, init_state=None,):
    '''
    ...
    if init_state is None:
        with Stopwatch("creating  variable") as stopwatch:
            ...
            tf_model = tf.squeeze(smpl(tf_trans,
                                    tf.concat((mean_tf_shape, tf_exp), axis=-1),
                                    tf.concat((tf_rot, tf_pose), axis=-1)))
    else:
        tf_model, tf_pose, tf_rot, tf_trans, mean_tf_shape, tf_exp = init_state
    ...
    with tf.Session() as session:
        ...
        # shape_reg = tf.reduce_sum(tf.square(mean_tf_shape))
        ...
        # rigid_pose_model = session.run(tf_model)

        # Optimize for the model parameters
        vars = [tf_trans, tf_rot, tf_pose, tf_exp]
        loss = weights['data'] * mesh_dist + weights['expr'] * exp_reg + \
        ...
        previous_state_variable = (tf_model, tf_pose, tf_rot, tf_trans, mean_tf_shape, tf_exp)

        return session.run(tf_model), session.run(tf_pose), session.run(tf_rot), session.run(tf_trans), session.run(mean_tf_shape), session.run(tf_exp), previous_state_variable
        # return rigid_pose_model, session.run(tf_pose), session.run(tf_rot), session.run(tf_trans), session.run(mean_tf_shape), session.run(tf_exp), previous_state_variable
